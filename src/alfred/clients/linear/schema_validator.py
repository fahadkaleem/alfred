...
from .utils import call_linear_api
...
def compare_fields(
    ...
    if (
        hasattr(model_class, "known_missing_fields")
        and model_class.known_missing_fields
    ):
...
def validate_model(model_class: type[LinearModel], api_key: str) -> Dict[str, Any]:
    ...
    try:
        ...
        known_extra = (
            model_class.known_extra_fields
            if hasattr(model_class, "known_extra_fields")
            else []
        )
        ...
        return {
            ...
            "completeness": (
                len(common) / (len(common) + len(missing) + len(known_missing))
                if common or missing or known_missing
                else 1.0
            ),
        }
...
def validate_all_models(api_key: str) -> Dict[str, Dict[str, Any]]:
    ...
    for model_class in models_to_validate:
        try:
            results[model_class.__name__] = validate_model(model_class, api_key=api_key)
        except Exception as e:
...
def get_field_details(
    graphql_type_name: str, api_key: str
...
def suggest_model_improvements(model_class: type[LinearModel], api_key: str) -> str:
    ...
    known_missing = (
        set(model_class.known_missing_fields)
        if hasattr(model_class, "known_missing_fields")
        else set()
    )
    ...
    if known_missing:
        suggestions.append(
            f"# Note: {len(known_missing)} known missing fields are excluded from suggestions"
        )
        suggestions.append(
            f"# Known missing fields: {', '.join(sorted(known_missing))}"
        )
