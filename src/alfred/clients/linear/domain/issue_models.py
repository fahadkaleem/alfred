...
class LinearIssue(LinearModel):
    ...
    def needs(self) -> List["CustomerNeedResponse"]:
        ...
        if hasattr(self, "_client") and self._client:
            ...
            print(
                f"Warning: Cannot fetch needs, no manager reference available for issue {self.id}"
            )
        return []
...
class LinearIssueInput(LinearModel):
    ...
    projectMilestoneName: Optional[str] = (
        None  # Will be converted to projectMilestoneId
    )
