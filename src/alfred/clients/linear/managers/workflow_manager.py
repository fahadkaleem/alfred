...
class WorkflowStateManager(BaseManager[LinearState]):
    ...
    def discover_team_states(
        ...
        team_states = [
            state for state in all_states if state.get("team", {}).get("id") == team_id
        ]
        ...
        result = TeamWorkflowStates(
            team_id=team_id, team_name=team_name, states=workflow_states
        )
    ...
    def discover_all_teams_states(
        ...
        for team_id, team in teams.items():
            try:
                ...
                print(
                    f"Warning: Could not fetch workflow states for team {team.name}: {e}"
                )
                continue
