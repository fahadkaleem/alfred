...
class TeamManager(BaseManager[LinearTeam]):
    ...
    def get_states(
        self, team_id: str, include_issue_ids: bool = False, force_refresh: bool = False
    ) -> List[LinearState]:
        """
       ...
       """
        ...
        state_nodes = self._handle_pagination(
            query, {"teamId": team_id}, ["workflowStates", "nodes"]
        )
        ...
        for state_data in state_nodes:
            if (
                include_issue_ids
                and "issues" in state_data
                and "nodes" in state_data["issues"]
            ):
                ...
                if issues_page_info.get("hasNextPage", False):
                    ...
                    more_issue_ids = self._get_all_issue_ids_for_state(
                        state_data["id"], issues_page_info["endCursor"]
                    )
    ...
    def get_membership(
        ...
        try:
            ...
            response = self._execute_query(
                query, {"teamId": team_id, "userId": user_id}
            )

            if (
                not response
                or "team" not in response
                or not response["team"]
                or "membership" not in response["team"]
                or not response["team"]["membership"]
            ):
                return None
            ...
            print(
                f"Failed to get membership for team {team_id} and user {user_id}: {e}"
            )
            return None

    def get_labels(
        self, team_id: str, include_issue_ids: bool = False
    ) -> List[LinearLabel]:
        """
        ...
        label_nodes = self._handle_pagination(
            query, {"teamId": team_id}, ["issueLabels", "nodes"]
        )
        ...
        for label_data in label_nodes:
            if (
                include_issue_ids
                and "issues" in label_data
                and "nodes" in label_data["issues"]
            ):
                ...
                if issues_page_info.get("hasNextPage", False):
                    ...
                    more_issue_ids = self._get_all_issue_ids_for_label(
                        label_data["id"], issues_page_info["endCursor"]
                    )
                    issue_ids.extend(more_issue_ids)
    ...
    def get_cycles(self, team_id: str) -> List[Dict[str, Any]]:
       ...
       """
        ...
        cycles = self._handle_pagination(
            query, {"teamId": team_id}, ["cycles", "nodes"]
        )
    ...
    def get_templates(self, team_id: str) -> List[Dict[str, Any]]:
        ...
        try:
           ...
           """
            ...
            response = self._execute_query(
                team_query, {"teamId": team_id, "cursor": None}
            )
            ...
            templates = self._handle_pagination(
                team_query, {"teamId": team_id}, ["team", "templates", "nodes"]
            )
    ...
    def get_issues(self, team_id: str) -> List[Dict[str, Any]]:
        ...
        issues = self._extract_and_cache(
            response, ["team", "issues"], "issues_by_team", team_id
        )
    ...
    def get_webhooks(self, team_id: str) -> List[Dict[str, Any]]:
        ...
        webhooks = self._extract_and_cache(
            response, ["team", "webhooks"], "webhooks_by_team", team_id
        )
    ...
    def get_parent(self, team_id: str) -> Optional[LinearTeam]:
        ...
        if (
            ...
            or "team" not in response
            or not response["team"]
            or "parent" not in response["team"]
            or not response["team"]["parent"]
        ):
    ...
    def get_triage_responsibility(self, team_id: str) -> TriageResponsibility:
        ...
        if (
            ...
            or "team" not in response
            or not response["team"]
            or "triageResponsibility" not in response["team"]
        ):
            return {}
        ...
        if triage_data:
            ...
            # Cache the result
            self._cache_set(
                "triage_responsibility_by_team", team_id, triage_responsibility
            )
