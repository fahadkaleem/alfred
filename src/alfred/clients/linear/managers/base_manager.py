...
class BaseManager(Generic[T]):
                ...
                for node in nodes:
                    ...
                    if model_class:
                        try:
                            ...
                            logging.warning(
                                f"Error converting node to {model_class.__name__}: {e}"
                            )
            ...
            for i, item in enumerate(results):
                if isinstance(item, dict) and not isinstance(item, model_class):
                    try:
                        ...
                        logging.warning(
                            f"Error converting item to {model_class.__name__}: {e}"
                        )
    ...
    def _cache_set(
        self, cache_name: str, key: Any, value: Any, ttl: Optional[int] = None
    ) -> None:
        """
