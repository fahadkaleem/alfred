...
def test_issue_with_state_workflow(client, test_team_name):
    ...
    try:
        ...
        update_data = LinearIssueUpdateInput(stateName=done_state.name)
...
def test_comprehensive_issue_workflow(client, test_team_name):
    ...
    try:
        ...
        from alfred.clients.linear.domain import LinearAttachmentInput
...
def test_comprehensive_model_updates(client, test_team_name):
    ...
    try:
        ...
        if states:
            ...
            print(
                f"Selected target state: ID={target_state.id}, Name={target_state.name}, Type={target_state.type}"
            )

            # Update issue state to target state
            update_data = LinearIssueUpdateInput(stateName=target_state.name)
            ...
            print(
                f"Updated issue state: ID={updated_issue.state.id}, Name={updated_issue.state.name}"
            )
            ...
            state = next(
                (s for s in refreshed_states if s.id == updated_issue.state.id), None
            )

            print(
                f"Found refreshed state: ID={state.id if state else 'None'}, Name={state.name if state else 'None'}"
            )

            if state and state.issue_ids:
                print(f"Issue IDs in state: {state.issue_ids}")

                assert issue.id in state.issue_ids, (
                    f"Issue ID {issue.id} not found in state's issue_ids: {state.issue_ids}"
                )
            else:
                print(f"State has no issue_ids: {state}")
                assert state is not None, (
                    f"Could not find state with ID {updated_issue.state.id}"
                )
...
def test_client_reference_enrich(client, test_team_name, test_project):
    ...
    # Create a test issue
    from alfred.clients.linear.domain import LinearIssueInput

    issue_input = LinearIssueInput(
        title=f"Test Client Reference {int(time.time())}", teamName=test_team_name
    )
