...
from alfred.clients.linear import LinearClient, LinearUser
from alfred.clients.linear.domain import LinearTeam, LinearState
...
def test_get_label_children(client, test_team_name):
    ...
    parent_label = next(
        (label for label in labels if getattr(label, "isGroup", False)), None
    )
